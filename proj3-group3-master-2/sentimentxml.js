	...
	if (this.readyState === this.DONE) {
		...
		if (aboveTen == false) {
			for (var i = 0; i < sentimentCounter; ++i) {			//push all sentiment values recieved
				...
				retainedNeg += res.documents[i].confidenceScores.negative;
				retainedNeu += res.documents[i].confidenceScores.neutral;
				retainedPos += res.documents[i].confidenceScores.positive;
				retainedTweetCount++;
			}
			isRunning = false;
...
function updateSentiment(tweets) {
	...
	xhr2.open("POST", "https://microsoft-text-analytics1.p.rapidapi.com/sentiment");
